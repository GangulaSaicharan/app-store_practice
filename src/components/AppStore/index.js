...
class AppStore extends Component {
  state = {tab: tabsList[0].tabId, searchInput: ''}
  ...
  getTabApps = () => {
    ...
    const tabAppsList = appsList.filter(eachApp => eachApp.category === tab)
    return tabAppsList
  }
  ...
  render() {
    const {tab, searchInput} = this.state
    const tabApps = this.getTabApps(tab)
    const filteredApps = tabApps.filter(each =>
